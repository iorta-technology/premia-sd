...
const EnhanceAvailmentComponent = React.memo((props) => {
  ...
  const enhancementAvailData = useSelector(
    (state) => state?.planTermination?.planTerminationData?.eab?.P_LOP_SURR_DTLS
  );
  console.log("enhancementAvailData", enhancementAvailData);
        ...
        <Row gutter={22} style={{ margin: "0", padding: 0 }}>
          <Col sm={24} md={18} lg={18} xlg={8} style={{padding: 0}}>
            ...
            <table>
              ...
              <tbody>
                {enhancementAvailData?.length > 0 ? (
                  enhancementAvailData.map((el) => {
                    return (
                      <tr>
                        <td data-label="date">
                          {el.SDFV_DATE
                            ? moment(el.SDFV_DATE).format("MMM DD, YYY")
                            : "--"}
                        </td>
                        <td data-label="amount">{el.SDFV_LC_GROSS_AMT}</td>
...
export default EnhanceAvailmentComponent;
