...

            {
              viewDetails.length ?
                ...
                viewDetails?.map((item, ind) => {
                  ...
                  return (
                    <div >
                      <div key={item.id} style={{ display: 'flex', backgroundColor: '#F7FBFF', height: '8rem', width: 'auto', marginBottom: '1rem', position: 'relative', alignItems: 'center' }}>
                        <Avatar style={{ textTransform: 'uppercase', position: 'absolute', left: '5px', top: '20px' }} size={{ xl: 50 }}>
                          {avatar}
                        </Avatar>
                        <p style={{ color: 'rgb(0, 172, 193)', fontSize: '13px',fontWeight:'bold', position: 'absolute', left: '14px', top: '70px' }}>
                          Agent
                        </p>
                        <p style={{ fontWeight: '700', position: 'absolute', left: '70px', top: '35px' }}>
                          {item.firstName} {item.lastName}
                        </p>
                        <p style={{ textTransform: 'uppercase', color: '#78849E', fontSize: '11px', position: 'absolute', left: '70px', top: '55px' }}>
                          {item.lead_Id}
                        </p>
                        <div style={{ position: 'absolute', top: '100px', left: '14px' }}>
                          <p>Reports to:  {item?.lead_allocated_by?.first_name} {item?.lead_allocated_by?.last_name}</p>
                        </div>
                      </div>

                      <div style={{ backgroundColor: '#F4F6F9', border: '1px solid #D2DDE8', padding: '10px', marginLeft: '15px', marginRight: '15px', height: '2.5rem' }}>
                        <p style={{  fontWeight: 'bold', color: 'rgb(0, 172, 193)' }}>Lead</p>

                      </div>
                      <div style={{ backgroundColor: '#F4F6F9', border: '1px solid #D2DDE8', padding: '10px', marginLeft: '15px', marginRight: '15px', height: '6rem' }}>
                        <p>Converted Lead</p> <p style={{ position: 'absolute', left: '40rem', bottom: '18rem' }}>0</p>
                        <p>Open</p> <p style={{ position: 'absolute', left: '40rem', bottom: '15.5rem' }}>0</p>
                      </div>
