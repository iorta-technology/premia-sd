...
const Index = ({ Remove, CurentOrPast, pastData, teamPast,pastDataln,teampastData }) => {
  ...
  return (
    ...
    pastData?.length>0 ? pastData?.map((element,index)=>{
      return(
    ...
    teampastData?.length>0 ? teampastData?.map((element,index)=>{
      return(
          <div className='dataField-Card' key={index}>
            {/* {
            ((1 + new Date(element.start_time_MS).getDate()) >= (1 + new Date().getDate())) 
