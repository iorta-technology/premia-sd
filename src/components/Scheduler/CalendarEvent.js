...
import Activity_Tracker from "../Scheduler/Activity_Tracker_ToDo_View/Activity_Tracker";
...
export default function CalendarEvent(props) {
  ...
  // const [event_created, setEvent_created] = useState('');
  ...
  const customerSearch = async () => {
    ...
    if (result.length > 0) {
      ...
      result[0].map((el) => {
        let sortarray = {
          _Id: el._id,
          value: toCapitalize(el.opportunity_name),
          // el.lastName +
          // " ( " +
          // el.lead_Id +
          // " )",
  ...
  const removeOpp = (data, ind) => {
    // console.log('removeTeamMember', data);
    // console.log('ownerCollectn=====>>', ownerCollectn);
    let _arrayOwner = oppCollectn.filter((item, index) => item.value !== data);
    console.log(_arrayOwner, "after removing");
  ...
  const BookAppointmentFunc = async (e) => {
    ...
    if (updateEventCheck == true) {
      ...
      // } 
        ...
        message.warn(' Appointment Update is in progress');
        props.setIsModalVisible(false);
      ...
      // } 
        ...
        if (eventDurationType == "customedatetime") {
          if (
            ...
            // setEvent_created('appointment is created');
            props.callback1();
            if (result.length !== 0) {
              if (props.api != undefined) props.api();
              if (props.getdata) props.getdata(true);
          ...
          let result = await axiosRequest.post(API,
            ...
            message.success('Appointment is created');
            console.log(res);
          })
          if (result.length !== 0) {
            // setEvent_created(result.erMsg);
  ...
  const changeCompanyName = async (value, compId) => {
    ...
    let _opportunityAPI = await axiosRequest.get(
      `user/opportunity/distinct/opportunity_names?company_id=${compId}`,
      {
        secure: true,
      }
    );
