...
const KpiDashboard = () => {
    ...
    if (_kpiResp.length > 0) {
      if (_kpiResp.length > 1) {
        ...
        for (let i = 0; i < _kpiResp.length; i++) {
          if (_kpiResp[i].hasOwnProperty("category"))
            gpwDataKey = _kpiResp[i].category;
          // console.warn('((((((((( gpwDataKey THAN )))))))))',gpwDataKey)
          if (_kpiResp[i].id === "current_month") {
            let currentData = checkValidity(_kpiResp[i].data);

            currentMonthData.currentDate = checkValidityCurrent(
              formateDate(currentData.uploadedDate)
            );
            currentMonthData.currentMonth = checkValidityCurrent(
              toCapitalize(currentData.month)
            );
            currentMonthData.currentYear = checkValidityCurrent(
              currentData.year
            );
            currentMonthData.GPW_Actual = checkValidityCurrent(
              currentData.GPW.gpw_actual
            );
            currentMonthData.parcentIssuance = checkValidity(
              currentData.parcentIssuance
            );
            currentMonthData.parcentPendancy = checkValidityCurrent(
              currentData.parcentPendancy
            );
            currentMonthData.parcentUnallocated = checkValidityCurrent(
              currentData.parcentUnallocated
            );
            currentMonthData.branch_activation_actual = branchDataHandling(
              currentData,
              "branch"
            );
            currentMonthData.gwp_retention_actual = branchDataHandling(
              currentData,
              "gwp"
            );
            ...
            let _data = dailyDataArray.map((el) => {
              ...
              el.gwpData =
                el.title === "Total GWP in ₹ Lac"
                  ? currentMonthData.GPW_Actual
                  : el.title === "Active Branches"
                  ? currentMonthData.branch_activation_actual
                  : el.title === "Total GWP Retention in ₹"
                  ? currentMonthData.gwp_retention_actual
                  : el.title === "% Issuance"
                  ? currentMonthData.parcentIssuance
                  : el.title === "Pendancy(GWP Pendancy vs. GWP Ach)"
                  ? currentMonthData.parcentPendancy
                  : el.title === "GWP Unallocated in ₹"
                  ? currentMonthData.parcentUnallocated
                  : "0";
              return el;
            });
            ...
            setEmployeeName(
              _kpiResp[i].data.first_name + " " + _kpiResp[i].data.last_name
            );
            setAvatarData(
              _kpiResp[i].data.first_name.match(/\b(\w)/g) +
                _kpiResp[i].data.last_name.match(/\b(\w)/g)
            );
        ...
        setEmployeeName(
          _kpiResp[0].data.first_name + " " + _kpiResp[0].data.last_name
        );
        setAvatarData(
          _kpiResp[0].data.first_name.match(/\b(\w)/g) +
            _kpiResp[0].data.last_name.match(/\b(\w)/g)
        );
    ...
    try {
      ...
      for (let _data of dataFinal) {
        for (let i = 0; i < _data.data.length; i++) {
          ...
          finalDataStruct.month = checkValidity(
            toCapitalize(shortMonth(_data.data[i].month))
          );
          finalDataStruct.monthTbl = checkValidity(
            toCapitalize(_data.data[i].month)
          );
          finalDataStruct.sales = checkValidity(
            parseFloat(_data.data[i].Final_Score)
          );
          finalDataStruct.index = checkValidity(
            parseFloat(_data.data[i].index)
          );
          finalDataStruct.change_in_percent = checkValidity(
            _data.data[i].change_in_percent.toPrecision(4)
          );
          finalDataStruct.year = checkValidity(_data.data[i].year);
          finalDataStruct.graphIND =
            _data.id === "final_score_last_six_month" ? "2" : "1";
  ...
  const gpwGraphData = (dataGPW, monthKey) => {
    try {
      ...
      for (let i = 0; i < dataGPW.data.length; i++) {
        for (let _dataKeys in dataGPW.data[i]) {
          if (dataGPW.category === _dataKeys) {
            ...
            for (let _gpwKeys in gpwTblData) {
              ...
              if (_gpwKeys.includes("achievement") === true)
                achieveKey = _gpwKeys;
            }
          }
        }
        ...
        gpwStruct.month = checkValidity(
          toCapitalize(shortMonth(dataGPW.data[i].month))
        );
  ...
  const onChangeKPIBudgetHandler = (value) => {
    let _channelId = login_user_data.channelCode._id;
    let _userId =
      teamData === "Select" || teamData === "" ? login_user_data.id : teamData;
          ...
          <Col xs={12} sm={12} md={12} lg={3} xl={3} style={{ marginLeft: 10 }}>
            {TeamSelf === false && (
              <Select
                ...
                onChange={(event) => handleDesignationData(event)}
            ...
            <Row  gutter={[10, 10]}>
              ...
              <Col xs={24} sm={24} md={22} lg={22} xl={22}>
                <Row
                  ...
                  style={{ flexWrap: "wrap", marginLeft: 10 }}
                >
